 ...
 **********************/
typedef struct {
    // Physics properties (keep float for smooth movement)
    float dino_vy;       /* Vertical velocity (positive = upward, negative = downward) */
    float dino_y;        /* Vertical position (0 = ground level) */
    float dino_vx;       /* Horizontal velocity */
    float dino_x;        /* Horizontal position */

    // Obstacle positions (use int16_t to save memory - screen coords are small)
    int16_t obstacle_x;      /* Horizontal position of ground obstacle */
    int16_t air_obstacle_x;  /* Horizontal position of air obstacle */

    // Game counters (use appropriate sized integers)
    ...
    uint8_t initialized : 1;         /* Boolean: 1 when game is fully initialized */
    uint8_t paused : 1;              /* Boolean: 1 when game is paused */
    uint8_t show_exit_dialog : 1;    /* Boolean: 1 when exit confirmation dialog is shown */
...
void dino_game_show(void)
{
    ...
    g_gs.obstacle_x = lv_obj_get_width(g_game_screen) + 50;
    ...
    g_air_obstacle = lv_gif_create(g_game_screen);
    ...
    g_gs.air_obstacle_x = lv_obj_get_width(g_game_screen) + 50;
    lv_obj_set_x(g_air_obstacle, g_gs.air_obstacle_x);
    lv_obj_set_y(g_air_obstacle, lv_obj_get_height(g_game_screen) - 30 - obstacle_h - 90); // 90px above ground obstacle
...
static void dino_game_show_exit_dialog(void)
{
    ...
    lv_coord_t btn_spacing = 50;  // Space between buttons (center to center) - increased for wider spacing
    ...
    lv_obj_align(g_exit_no_btn, LV_ALIGN_BOTTOM_MID, -btn_spacing, -5);  // Left of center, moved down further
    ...
    lv_obj_align(g_exit_yes_btn, LV_ALIGN_BOTTOM_MID, btn_spacing, -5);  // Right of center, moved down further
...
static void dino_game_show_game_over_dialog(void)
{
    ...
    lv_obj_align(g_game_over_yes_btn, LV_ALIGN_BOTTOM_MID, -btn_spacing, -5);  // Left of center
    lv_obj_set_style_bg_color(g_game_over_yes_btn, lv_color_black(), 0);  // Black background (selected)
    ...
    lv_obj_align(g_game_over_no_btn, LV_ALIGN_BOTTOM_MID, btn_spacing, -5);  // Right of center
    lv_obj_set_style_bg_color(g_game_over_no_btn, lv_color_white(), 0);  // White background (not selected)
...
static void dino_game_timer_cb(lv_timer_t *tmr)
{
    ...
    lv_coord_t screen_width = lv_obj_get_width(g_game_screen);
